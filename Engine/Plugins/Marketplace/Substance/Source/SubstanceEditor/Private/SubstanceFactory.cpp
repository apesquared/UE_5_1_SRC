...
void UReimportSubstanceFactory::SaveRecreationData(USubstanceInstanceFactory* FactoryToReimport)
{
	...
	for (const auto& GraphItr : FactoryToReimport->GetGraphInstances())
	{
		...
		for (TObjectIterator<UMaterial> MatItr; MatItr; ++MatItr)
		{
			...
			for (int32 ExpressionIndex = (*MatItr)->GetExpressionCollection().Expressions.Num() - 1; ExpressionIndex >= 0; ExpressionIndex--)
			{
				UMaterialExpressionTextureSample* Expression = Cast<UMaterialExpressionTextureSample>((*MatItr)->GetExpressionCollection().Expressions[ExpressionIndex]);
